...
add_custom_target(setup_runtime_libraries ALL
    COMMAND ${CMAKE_COMMAND} -E env bash -c "
        if [ ! -f ${TI_CGT_ROOT}/lib/rtsv7R4_T_le_v3D16_eabi.lib ]; then \
            PATH=$ENV{PATH}:${TI_CGT_ROOT}/bin \
            ${TI_CGT_ROOT}/lib/mklib --pattern rtsv7R4_T_le_v3D16_eabi.lib > /dev/null 2>&1; \
        else \
...
add_compile_definitions(-DSOC_XWR68XX -DSUBSYS_MSS)
...
if(USE_TI_RTOS)
    ...
    set(TARGET_PLATFORM "\"ti.targets.arm.elf.R4F\"")
    set(PLATFORM "ti.platforms.cortexR:IWR68XX:false:200")

    # Compiler options
    set(COMPILE_OPTIONS "\"--enum_type=int\"")

    # Add custom command to run XDCtools
    add_custom_command( OUTPUT "${XDC_OUTPUT_DIR}/package"
                        ...
                        ARGS --xdcpath="${TI_RTOS_PATH}/packages\;"
                             ...
                             --compileOptions "${COMPILE_OPTIONS}"
                                              "${TI_RTOS_CONFIG}"
                             WORKING_DIRECTORY ${CMAKE_BINARY_DIR} # Optional: Specify where the command runs
                             DEPENDS ${TI_RTOS_CONFIG}                # Re-run if config changes
                             COMMENT "Running XDCtools configuro"
                      )

    # Create a custom target for XDCtools
    add_custom_target(xdc_gen ALL DEPENDS "${XDC_OUTPUT_DIR}/package")
    add_link_options(-l${XDC_OUTPUT_DIR}/linker.cmd)
endif()

function(generate_bin_file build_target)
    add_custom_command(
        TARGET ${build_target}
        POST_BUILD
        COMMAND ${TI_MMWAVE_PATH}/packages/scripts/ImageCreator/out2rprc/out2rprc.exe ${CMAKE_BINARY_DIR}/${build_target} ${CMAKE_BINARY_DIR}/${build_target}.bin
        COMMENT "Generate binary file of the link output"
    )
endfunction()

function(generate_multicore_image_file build_target)
    add_custom_command(
    TARGET ${build_target}
    POST_BUILD
    COMMAND ${TI_MMWAVE_PATH}/packages/scripts/ImageCreator/multicore_image_generator/MulticoreImageGen LE 37 0x00000006 out_image.bin 0x35510000 ${CMAKE_BINARY_DIR}/${build_target}.bin 0xb5510000 ${TI_MMWAVE_PATH}/firmware/radarss/xwr6xxx_radarss_rprc.bin 0xd5510000 ${CMAKE_BINARY_DIR}/image_dss.bin
            && ${TI_MMWAVE_PATH}/packages/scripts/ImageCreator/crc_multicore_image/crc_multicore_image ${CMAKE_BINARY_DIR}/${build_target}.bin ${CMAKE_BINARY_DIR}/${build_target}.bin.tmp
            && ${TI_MMWAVE_PATH}/packages/scripts/ImageCreator/append_bin_crc/gen_bincrc32 ${CMAKE_BINARY_DIR}/${build_target}.bin
    COMMENT "Generate binary file of the link output")
endfunction()
