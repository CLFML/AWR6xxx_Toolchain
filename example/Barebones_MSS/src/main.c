/*
*  Barebones MSS example: bare-metal (no SYS/BIOS) boot on the AWR6843AOP.
*
*  Third bare-metal attempt on this board -- see git history and the
*  AWR6xxx_Toolchain memory notes for the first two, which both hit a
*  reliable data abort on the first MSS peripheral register access after
*  boot. This one explicitly replicates the two pieces of hardware
*  bring-up SYS/BIOS's own generated startup does that a plain TI RTS
*  _c_int00 does not -- see src/startup_awr6843.asm's file header and the
*  AWR6xxx_Toolchain memory note on xdctools/configuro's generated output
*  for the full story: initializing the Abort/Undefined-mode banked stack
*  pointers, and resetting VIM/clearing ESM, both before main() runs.
